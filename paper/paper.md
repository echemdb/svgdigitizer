...
Several tools are available for extracting numerical data from published plots, ranging from web-based interfaces to windows-only desktop and cross-platform applications, and spanning both open-source and commercial licenses. Examples include WebPlotDigitizer [@rohatgi_2022], starry-digitizer (buildin on WebPlotDigitizer),[@KATSURA2025] Engauge Digitizer [@mark_mitchell_2020_3941227], and OriginPro [@origin_2026]. These tools typically provide interactive workflows for recovering data points from graphical representations through manual selection or semi-automated detection methods, enabling plotted information to be converted into reusable numerical datasets. These tools are
well-suited to one-off, GUI-driven data extraction. However, these tools usually do not allow exporting physical units,
neither produces structured metadata, or offers a programmable Python API for
integration into automated workflows.

Following we briefly summarize the current lmitations of the available tools.
<!-- AKE: Not sure if the svgdigitizer aspects should be highlighted here, since it should be the state of the field. -->
...
work with raster images where individual data points are selected on the curve by mouse clicks and to some extend by auto detection methods, which then present the final dataset. starry-digitizer allows interpolating between selected datapoints to get more fine grained dataset.
<!-- AKE: Not sure if the svgdigitizer aspects should be highlighted here, since it should be the state of the field. -->
<!-- `svgdigitizer` in turn leverages the SVG format in which still a small set of points must be selected manually on a curve, which are then, however connected by Bézier paths.-->
...
the other scientific ecosystems, such as Python,<!--REF PYTHON--> and the development of domain specific modules to extract specific types of data according to pre defined standards.
<!-- AKE: Not sure if the specific modules should be highlighted here.-->
<!--ecosystem — `astropy.units` [@astropy_2022_astropy_167] for unit parsing, `pandas` [@pandas_2026_pandas_18675244] for
...
The standard output file of svgdigitizer are frictionless datapacakges, which consist of a CSV (including the curve data) and a JSON (providing information on the axis units and additional metadata from the SVG).
During the creation of the datapackage, additional metdata can be added from external YAML and BibTeX files into the datapackage descriptor,
...
Claude Sonnet 4.6 (Anthropic) as a writing aid from the documentation of the svgdigitizer project. All content was reviewed, revised, and
